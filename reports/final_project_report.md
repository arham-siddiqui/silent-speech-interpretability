...
across recording sessions at **28.7%** accuracy, but
leave-one-subject-out performance falls to **3.0%**,
...
| External session transfer | Three seeds, held-out sessions | **28.7%** accuracy | Supported |
| External speaker transfer | Three seeds, held-out subjects | **3.0%** accuracy | Not supported |
...
Session-held-out radar accuracy is **28.7%**
[19.9%,
37.0%] versus 2% chance.
True-order HuBERT cosine is **0.327**, and the
true-minus-reversed margin is **+0.339**. After removing
train-only command prototypes, the residual order margin remains
**+0.122**.

In contrast, subject-held-out accuracy is **3.0%**
[2.6%,
3.4%]. HuBERT cosine is
**-0.014**, and order margin is **-0.012**,
...
S32 alone retains **28.2%** accuracy,
essentially matching the combined input for seed 42. Magnitude-only features reach
**27.1%**, while delta-only and
S12-only features fall to **16.6%**
and **14.5%**. S32 magnitude structure
