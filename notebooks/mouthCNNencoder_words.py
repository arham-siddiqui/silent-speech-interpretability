...
def make_loaders(X, y, users):
    tr = np.where(~np.isin(users, VAL_USERS + TEST_USERS))[0]
    va = np.where(np.isin(users, VAL_USERS))[0]
    te = np.where(np.isin(users, TEST_USERS))[0]

    print(f"Split (user-based): {len(tr)} train | {len(va)} val | {len(te)} test")
...
def main():
    ...
    results = {
        ...
        "split": {"train_users": "1-16", "val_users": "17-18",
                  "test_users": "19-20",
                  "train_n": len(tr_idx), "val_n": len(va_idx),
                  "test_n": len(te_idx)},
